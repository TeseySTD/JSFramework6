export class Validator{
    ...
    static validateForm(form: HTMLFormElement): boolean {
        ...
        if(formName && formDob && formEmail && formPhone) {
            isValid = this.validateName(formName.value) && this.validateDob(new Date(formDob.value)) 
                        && this.validateEmail(formEmail.value) && this.validatePhone(formPhone.value);
            
            inputs.forEach(input => {input.classList.remove("is-valid"); input.classList.remove("is-invalid");});
